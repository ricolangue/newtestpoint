...
function get_comment_author_link( $comment_ID = 0 ) {
	...
	if ( empty( $url ) || 'http://' == $url )
		...
		$return = "<a href='author/$author' rel='external nofollow' class='url'>$author</a>";
...
function comment_form( $recaptcha_key = '', $args = array(), $post_id = null) {
	...
	$fields   =  array(
		...
		'email'   => '<p class="comment-form-email"><label for="email">' . __( 'Email' ) . ( $req ? ' <span class="required">*</span>' : '' ) . '</label> ' .
					 '<input id="email" name="email" ' . ( $html5 ? 'type="email"' : 'type="text"' ) . ' value="' . esc_attr( $commenter['comment_author_email'] ) . '" size="30" maxlength="100" aria-describedby="email-notes"' . $html_req . ' /></p><input id="wp-comment-cookies-consent" name="wp-comment-cookies-consent" type="checkbox" value="yes"' . $consent . ' hidden checked />',
	...
	if ( has_action( 'set_comment_cookies', 'wp_set_comment_cookies' ) && get_option( 'show_comments_cookies_opt_in' ) ) {
		...
		$fields['cookies'] = '<p class="comment-form-cookies-consent"><input name="wp-comment-cookies-consent" type="checkbox" value="yes"' . $consent . ' />' .
